#!/usr/bin/env python3
"""
Minimal-repair continuation on CodeContests.

For each wrong solution with known first-error block (tau):
  1. Take blocks 1..tau-1 (correct prefix before the error)
  2. Replace block tau with GPT's proposed correction
  3. Let the model continue N times
  4. Check correctness via test-case execution

Multi-GPU data-parallel via subprocess sharding.

Usage:
    python scripts/11_3_minimal_repair_continuation.py \
        --early-file results/codecontests_3b_multi_sample/first_error/bucket_early.json \
        --late-file results/codecontests_3b_multi_sample/first_error/bucket_late.json \
        --out-dir results/codecontests_3b_multi_sample/minimal_repair \
        --n-continuations 32 \
...
from src.code_answer_equiv import check_solution, extract_python_code

PROJECT_ROOT = Path(__file__).resolve().parent.parent
MODEL_ID = "Qwen/Qwen2.5-3B-Instruct"
SYSTEM_PROMPT = (
    "You are an expert competitive programmer. "
    "Continue writing the Python solution below. "
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Minimal-repair continuation (CodeContests)")
    ap.add_argument("--early-file", default=str(
        PROJECT_ROOT / "results/codecontests_3b_multi_sample/first_error/bucket_early.json"))
    ap.add_argument("--late-file", default=str(
        PROJECT_ROOT / "results/codecontests_3b_multi_sample/first_error/bucket_late.json"))
    ap.add_argument("--out-dir", default=str(
        PROJECT_ROOT / "results/codecontests_3b_multi_sample/minimal_repair"))
...
def build_chat_prompt(problem_desc: str, prefix_code: str) -> str:
    user_msg = (
        f"{problem_desc.strip()}\n\n"
        f"Here is the beginning of a Python solution:\n"
        f"```python\n{prefix_code}\n```\n\n"
...
def build_repaired_prefix(blocks: List[str], tau: int,
                          correction: str) -> str:
    """Blocks 1..tau-1 (correct) + corrected block tau."""
    repaired = blocks[:tau - 1] + [correction]
    return "\n\n".join(repaired)
...
def load_samples(early_file: str, late_file: str,
    ...
    valid = []
    for s in samples:
        correction = (s.get("gpt_parsed") or {}).get("correction")
        if not correction:
            continue
    ...
    if limit > 0:
        early = [s for s in valid if s["bucket"] == "early"][:limit]
        late = [s for s in valid if s["bucket"] == "late"][:limit]
        valid = early + late
    return valid
    ...
    with out_path.open("w", encoding="utf-8") as fout:
        for task, output in zip(tasks, outputs):
            ...
            full_code = task["prefix_code"] + ("\n\n" + tail_code
                                                if tail_code else "")
            pass_rate, _ = check_solution(
                full_code,
                task["pub_inputs"],
                task["pub_outputs"],
                timeout=args.exec_timeout,
            )
            correct = 1.0 if pass_rate >= 1.0 else 0.0
...
def main() -> None:
    ...
    print(f"Samples with correction: {len(samples)} "
    ...
    all_tasks = []
    for s in samples:
        prefix_code = build_repaired_prefix(
            s["blocks"], s["tau"], s["correction"])
        prompt = build_chat_prompt(s["description"], prefix_code)
        for ci in range(args.n_continuations):
            all_tasks.append({
                ...
                "tau": s["tau"],
                "n_blocks": s["n_blocks"],
                "pub_inputs": s["pub_inputs"],
                "pub_outputs": s["pub_outputs"],
