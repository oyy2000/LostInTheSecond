#!/usr/bin/env python3
"""
Fine-grained relative-position analysis on MATH-500.
...
Usage:
    python scripts/9_5_fine_grained_relpos.py
"""
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--out-summary", default=str(
        PROJECT_ROOT / "results/math500_3b_multi_sample/fine_relpos_summary.json"))
...
def main():
    ...
    ax.set_title(f"MATH-500: Repair Gain by Position (KW p={kw_p:.2e})")
    ...
    ax.set_title(f"MATH-500: Spearman $\\rho_s$={rho:.3f}, p={p_rho:.1e}")
    ax.axhline(0, color="gray", linestyle="--", linewidth=0.8)
    ...
    ax.set_title("MATH-500: Pairwise Welch t-test p-values")
    plt.tight_layout()
