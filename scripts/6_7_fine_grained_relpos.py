#!/usr/bin/env python3
"""
Fine-grained relative-position analysis with pairwise p-values.
...
Usage:
    python scripts/6_7_fine_grained_relpos.py
"""
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--out-summary", default=str(
        PROJECT_ROOT / "results/gsm8k_3b_multi_sample/fine_relpos_summary.json"))
...
def main():
    ...
    # Aggregate per question (doc_id): average across samples
    ...
    # Panel A: bar chart with error bars and significance brackets
    ax = axes[0]
    ...
    for bar, m, n in zip(bars, means, ns):
        y_top = bar.get_height() + 1.96 * ses[bars.index(bar)] + 0.005
    ...
    ax.set_title(f"Repair Gain by Position (Kruskal-Wallis p={kw_p:.2e})")
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)

    # Add significance bracket for Q1 vs Q5
    ...
    # Panel B: scatter + finer binned means + trend line
    ax = axes[1]
    ...
    ax.set_title(f"Spearman $\\rho_s$={rho:.3f}, p={p_rho:.1e}")
    ax.axhline(0, color="gray", linestyle="--", linewidth=0.8)
    ...
    ax.set_title("Pairwise Welch t-test p-values")
    plt.tight_layout()
