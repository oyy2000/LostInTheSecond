#!/usr/bin/env python3
"""
Find first-error block (tau) in wrong HumanEval solutions via GPT,
then bucket by early/late tau.

Usage:
    python scripts/12_1_find_first_error_and_bucket.py \
...
def build_humaneval_error_prompt(
    func_prompt: str,
    entry_point: str,
    code_blocks: list,
) -> str:
...
Function specification (signature + docstring):
```python
{func_prompt.strip()}
```
...
def main():
    parser = argparse.ArgumentParser(
        description="GPT first-error locator + tau bucketing (HumanEval)")
    parser.add_argument(
        "--input", type=str,
        default="results/humaneval_3b_multi_sample/raw_code_n8.jsonl")
    parser.add_argument("--output-dir", type=str,
                        default="results/humaneval_3b_multi_sample/first_error")
    ...
    if args.dry_run:
        sample = wrong[0]
        prompt = build_humaneval_error_prompt(
            sample["prompt"], sample["entry_point"], sample["blocks"])
        print("\n=== DRY RUN ===")
        print(prompt)
        return
    ...
    if tasks and not args.bucket_only:
        ...
        def _process(row):
            prompt = build_humaneval_error_prompt(
                row["prompt"], row["entry_point"], row["blocks"])
