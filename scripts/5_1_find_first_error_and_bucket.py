...
# -*- coding: utf-8 -*-
"""
Find first-error step (tau) in wrong GSM8K trajectories via GPT 5.1,
then bucket by early/late tau with length constraints.

Pipeline:
  1. Load raw_cot_n8.jsonl, filter wrong-answer trajectories.
  2. For each wrong trajectory, call GPT 5.1 to locate first_error_step (tau)
     and propose a minimal correction.
  3. Cache results incrementally to JSONL.
  4. Compute tau, bucket into early (tau in {2,3}, n_steps>=3)
     and late (tau in {5,6}, n_steps>=6).
  5. Save bucketed outputs.

Usage:
    python scripts/5_1_find_first_error_and_bucket.py \
        --input results/gsm8k_3b_multi_sample/raw_cot_n8.jsonl \
        --output-dir results/gsm8k_3b_multi_sample/first_error \
...
def bucket_results(records: list) -> dict:
    """Assign each record to early / late / other bucket."""
    early, late, other = [], [], []
    for r in records:
        tau = r.get("tau")
        n = r.get("n_steps", 0)
...
def main():
    parser = argparse.ArgumentParser(
        description="GPT 5.1 first-error locator + tau bucketing"
    )
    parser.add_argument(
        "--input", type=str,
    ...
    parser.add_argument("--output-dir", type=str,
                        default="results/gsm8k_3b_multi_sample/first_error")
    parser.add_argument("--model", type=str, default="gpt-5.1")
    parser.add_argument("--max-workers", type=int, default=8)
    parser.add_argument("--temperature", type=float, default=0.0)
    parser.add_argument("--max-samples", type=int, default=None,
                        help="Limit number of wrong trajectories to process")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--bucket-only", action="store_true",
                        help="Skip API calls, just re-bucket from cache")
    ...
    # Reload cache and merge original fields
    cache = load_cache(cache_path)
