#!/usr/bin/env python3
"""
Fine-grained relative-position analysis on CommonsenseQA.

Splits samples into 5 quantile bins by rel_pos = tau / n_steps,
computes per-bin repair gain (Delta_fix), and runs pairwise
Welch t-tests + Mann-Whitney U between all bin pairs.

Usage:
    python scripts/10_5_csqa_fine_relpos.py
"""
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--out-summary", default=str(
        PROJECT_ROOT / "results/csqa_3b_multi_sample/fine_relpos_summary.json"))
...
def main():
    ...
    per_q = defaultdict(lambda: {"rel_pos": [], "r_bad": [], "r_fix": [], "delta": []})
    for key in common:
        doc_id = key[0]
        ...
        per_q[doc_id]["rel_pos"].append(tau / n_steps)
    ...
    if len(paired) < n_bins * 2:
        print(f"WARNING: only {len(paired)} data points, need at least {n_bins*2}")
        return
    ...
    for i in range(n_bins):
        mask = (rel_positions >= edges[i]) & (rel_positions < edges[i + 1])
        d = deltas[mask]
        rb = r_bads[mask]
        rf = r_fixs[mask]
    ...
    print(f"\n{'Bin':<6} {'Range':<22} {'n':>5}  {'Delta_mean':>10}  {'SE':>8}  {'R_bad':>8}  {'R_fix':>8}")
    print("-" * 80)
    for b in bin_data:
    ...
    for (i, bi), (j, bj) in combinations(enumerate(bin_data), 2):
        t_val, p_val = sp_stats.ttest_ind(bi["deltas"], bj["deltas"], equal_var=False)
        u_val, p_mw = sp_stats.mannwhitneyu(bi["deltas"], bj["deltas"], alternative="two-sided")
        ...
        print(f"  {key:<12}  t={t_val:>7.3f}  p={p_val:.3e}  {sig}   (MW p={p_mw:.3e})")
    ...
    jt_z = (jt_stat - jt_mean) / np.sqrt(jt_var) if jt_var > 0 else 0
    ...
    bars = ax.bar(x, means, yerr=[1.96 * s for s in ses],
                  capsize=4, color=colors, edgecolor="black", linewidth=0.6, width=0.65)
    ...
    ax.set_xlabel("Relative Error Position ($\\tau / N_{steps}$) Quantile")
    ax.set_title(f"CommonsenseQA: Repair Gain by Position (KW p={kw_p:.2e})")
    ...
    if key_15 in pairwise:
        ...
        sig_str = "***" if p15 < 0.001 else "**" if p15 < 0.01 else "*" if p15 < 0.05 else "ns"
    ...
    ax = axes[1]
    ax.scatter(rel_positions, deltas, alpha=0.1, s=10, color="#2c3e50", rasterized=True)
    ...
    ax.set_title(f"CommonsenseQA: Spearman $\\rho_s$={rho:.3f}, p={p_rho:.1e}")
    ax.axhline(0, color="gray", linestyle="--", linewidth=0.8)
    ...
    plt.tight_layout()
    fig.savefig(fig_dir / "fig_relative_position_fine.pdf", dpi=150, bbox_inches="tight")
    fig.savefig(fig_dir / "fig_relative_position_fine.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
    ...
    log_p = -np.log10(np.where(np.isnan(p_matrix), 1.0, np.clip(p_matrix, 1e-20, 1.0)))
    ...
    ax.set_title("CommonsenseQA: Pairwise Welch t-test p-values")
    plt.tight_layout()
    fig.savefig(fig_dir / "fig_relpos_pairwise_pvalues.pdf", dpi=150, bbox_inches="tight")
    fig.savefig(fig_dir / "fig_relpos_pairwise_pvalues.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
    ...
    summary = {
        ...
        "bins": [{k: v for k, v in b.items() if k != "deltas"} for b in bin_data],
