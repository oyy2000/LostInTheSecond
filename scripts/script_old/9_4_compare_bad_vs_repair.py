#!/usr/bin/env python3
"""
Combined analysis: Bad-prefix recovery vs Minimal repair on MATH-500.
...
Requires outputs from 9_2 and 9_3.

Usage:
    python scripts/9_4_compare_bad_vs_repair.py
    python scripts/9_4_compare_bad_vs_repair.py --per-question
"""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Compare bad-prefix vs repair (MATH-500)")
    ap.add_argument("--bad-file", default=str(
        PROJECT_ROOT / "results/math500_3b_multi_sample/bad_prefix_recovery/continuations.jsonl"))
    ap.add_argument("--fix-file", default=str(
        PROJECT_ROOT / "results/math500_3b_multi_sample/minimal_repair/continuations.jsonl"))
    ap.add_argument("--fig-dir", default=str(
        PROJECT_ROOT / "figures/math500_bad_prefix_recovery"))
    ap.add_argument("--out-summary", default=str(
        PROJECT_ROOT / "results/math500_3b_multi_sample/repair_gain_summary.json"))
...
def main():
    ...
    ax.set_title(f"MATH-500: Bad Prefix vs. Minimal Repair ({granularity})")
    ...
    ax.set_title("MATH-500: $R_{fix} - R_{bad}$")
    ...
    ax.set_title(f"MATH-500: Welch t={t_between:.2f}, p={p_between:.2e}")
    ax.axhline(0, color="gray", linestyle="--", linewidth=0.8)
    ...
    ax.set_ylabel("Recovery Rate")
    ax.set_title("MATH-500: Recovery with and without Repair")
    ax.set_xticks(taus)
    ax.legend(loc="upper left")
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ...
    ax.set_ylabel("Repair Gain $\\Delta_{fix}$")
    ax.set_title("MATH-500: Repair Gain by Relative Position")
