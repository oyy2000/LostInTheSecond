#!/usr/bin/env python3
"""
Combined analysis: Bad-prefix recovery vs Minimal repair on HumanEval.

Produces the same set of figures as 9_4 (MATH-500) but for code generation:
  - R_bad vs R_fix grouped bars
  - Delta by bucket
  - Violin + strip
  - Recovery curves by tau
  - Relative position analysis (tercile + scatter)

Usage:
    python scripts/12_4_compare_bad_vs_repair.py
    python scripts/12_4_compare_bad_vs_repair.py --per-question
"""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Compare bad-prefix vs repair (HumanEval)")
    ap.add_argument("--bad-file", default=str(
        PROJECT_ROOT / "results/humaneval_3b_multi_sample/bad_prefix_recovery/continuations.jsonl"))
    ap.add_argument("--fix-file", default=str(
        PROJECT_ROOT / "results/humaneval_3b_multi_sample/minimal_repair/continuations.jsonl"))
    ap.add_argument("--fig-dir", default=str(
        PROJECT_ROOT / "figures/humaneval_bad_prefix_recovery"))
    ap.add_argument("--out-summary", default=str(
        PROJECT_ROOT / "results/humaneval_3b_multi_sample/repair_gain_summary.json"))
    ap.add_argument("--per-question", action="store_true",
                    help="Aggregate by doc_id instead of (doc_id, sample_idx)")
...
def main():
    ...
    bad_rates = (load_per_question_rates(bad_path) if per_q
                 else load_per_sample_rates(bad_path))
    fix_rates = (load_per_question_rates(fix_path) if per_q
                 else load_per_sample_rates(fix_path))
    granularity = "per-question" if per_q else "per-sample"
    fig_suffix = "_perq" if per_q else ""
    print(f"Granularity: {granularity}")
    print(f"Bad-prefix items: {len(bad_rates)}, Fix items: {len(fix_rates)}")

    common_keys = set(bad_rates.keys()) & set(fix_rates.keys())
    print(f"Common items: {len(common_keys)}")
    ...
    for bucket in ["early", "late"]:
        ...
        summary[bucket] = {
            ...
            "delta_ttest_t": float(t_stat),
            "delta_ttest_p": float(p_val),
        }
        print(f"  {bucket} (n={n}):")
        print(f"    R_bad  = {np.mean(r_bad):.4f} +/- "
              f"{np.std(r_bad)/np.sqrt(n):.4f}")
        print(f"    R_fix  = {np.mean(r_fix):.4f} +/- "
              f"{np.std(r_fix)/np.sqrt(n):.4f}")
        print(f"    Delta  = {np.mean(delta):.4f} +/- "
              f"{np.std(delta)/np.sqrt(n):.4f} "
    ...
    t_between, p_between = sp_stats.ttest_ind(
        delta_early, delta_late, equal_var=False)
    u_stat, p_mann = sp_stats.mannwhitneyu(
        delta_early, delta_late, alternative="two-sided")
    pooled_std = np.sqrt(
        (np.var(delta_early, ddof=1) + np.var(delta_late, ddof=1)) / 2)
    cohens_d = ((np.mean(delta_early) - np.mean(delta_late)) / pooled_std
                if pooled_std > 0 else 0.0)
    print(f"  Welch t={t_between:.3f}, p={p_between:.2e}")
    ...
    between_test = {
        ...
        "welch_t": float(t_between), "welch_p": float(p_between),
        "mann_whitney_U": float(u_stat), "mann_whitney_p": float(p_mann),
        "cohens_d": float(cohens_d),
        "n_early": len(delta_early), "n_late": len(delta_late),
    }
    ...
    # -- Figure 1: Grouped bar R_bad vs R_fix --
    ...
    ax.bar(x - width/2, r_bad_means, width,
           yerr=[1.96*s for s in r_bad_ses],
           capsize=4, label="$R_{bad}$ (no repair)", color="#e74c3c",
           alpha=0.8)
    ax.bar(x + width/2, r_fix_means, width,
           yerr=[1.96*s for s in r_fix_ses],
           capsize=4, label="$R_{fix}$ (repaired)", color="#3498db",
           alpha=0.8)
    ax.set_xticks(x)
    ax.set_xticklabels([
        "early ($\\tau$=2,3)", "late ($\\tau$=4,5,6)"])
    ax.set_ylabel("Recovery Rate (pass@tests)")
    ax.set_title(f"HumanEval: Bad Prefix vs. Minimal Repair ({granularity})")
    ax.legend(loc="upper left")
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ax.set_ylim(0, max(r_bad_means + r_fix_means) * 1.4)
    plt.tight_layout()
    fig.savefig(fig_dir / f"fig_bad_vs_repair{fig_suffix}.pdf",
                dpi=150, bbox_inches="tight")
    fig.savefig(fig_dir / f"fig_bad_vs_repair{fig_suffix}.png",
                dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"\nSaved: {fig_dir / f'fig_bad_vs_repair{fig_suffix}.pdf'}")

    # -- Figure 2: Delta by bucket --
    fig, ax = plt.subplots(1, 1, figsize=(4, 4))
    delta_means = [summary[b]["delta_mean"] for b in buckets_list]
    delta_ses = [summary[b]["delta_se"] for b in buckets_list]
    colors = ["#e74c3c", "#2ecc71"]
    bars = ax.bar(buckets_list, delta_means,
                  yerr=[1.96*s for s in delta_ses],
                  capsize=5, color=colors, edgecolor="black",
                  linewidth=0.8, width=0.5)
    ax.set_ylabel("Repair Gain $\\Delta_{fix}$")
    ax.set_xlabel("Error Position")
    ax.set_title("HumanEval: $R_{fix} - R_{bad}$")
    ...
    ax.spines["right"].set_visible(False)
    plt.tight_layout()
    fig.savefig(fig_dir / f"fig_repair_gain{fig_suffix}.pdf",
                dpi=150, bbox_inches="tight")
    fig.savefig(fig_dir / f"fig_repair_gain{fig_suffix}.png",
                dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {fig_dir / f'fig_repair_gain{fig_suffix}.pdf'}")

    # -- Figure 3: Violin + strip --
    fig, ax = plt.subplots(1, 1, figsize=(5, 4))
    vp = ax.violinplot([delta_early, delta_late], positions=[0, 1],
                       showmedians=True, showextrema=False)
    for i, (body, color) in enumerate(
            zip(vp["bodies"], ["#e74c3c", "#2ecc71"])):
        body.set_facecolor(color)
        body.set_alpha(0.3)
    vp["cmedians"].set_color("black")
    rng = np.random.default_rng(42)
    for i, (data, color) in enumerate(
            zip([delta_early, delta_late], ["#e74c3c", "#2ecc71"])):
    ...
    ax.set_xticklabels([
        "early ($\\tau$=2,3)", "late ($\\tau$=4,5,6)"])
    ax.set_ylabel("Per-item Repair Gain $\\Delta_i$")
    ax.set_title(f"HumanEval: Welch t={t_between:.2f}, p={p_between:.2e}")
    ax.axhline(0, color="gray", linestyle="--", linewidth=0.8)
    for i, data in enumerate([delta_early, delta_late]):
        ax.plot(i, np.mean(data), "D", color="black", markersize=8, zorder=5)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    plt.tight_layout()
    fig.savefig(fig_dir / f"fig_between_bucket_delta{fig_suffix}.pdf",
                dpi=150, bbox_inches="tight")
    fig.savefig(fig_dir / f"fig_between_bucket_delta{fig_suffix}.png",
                dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: "
          f"{fig_dir / f'fig_between_bucket_delta{fig_suffix}.pdf'}")

    # -- Figure 4: Recovery curves by tau --
    ...
    ax.plot(taus, r_bad_tau, "o-", color="#e74c3c", linewidth=2,
            markersize=7, label="$R_{bad}$ (no repair)")
    ax.plot(taus, r_fix_tau, "s-", color="#3498db", linewidth=2,
            markersize=7, label="$R_{fix}$ (repaired)")
    ax.fill_between(taus, r_bad_tau, r_fix_tau, alpha=0.12, color="#3498db")
    for t, d in zip(taus, delta_tau):
        mid = (r_bad_tau[taus.index(t)] + r_fix_tau[taus.index(t)]) / 2
        ax.annotate(f"$\\Delta$={d:.3f}", (t, mid),
                    textcoords="offset points", xytext=(15, 0),
                    fontsize=8, color="#2c3e50")
    ax.set_xlabel("First Error Block ($\\tau$)")
    ax.set_ylabel("Recovery Rate (pass@tests)")
    ax.set_title("HumanEval: Recovery with and without Repair")
    ax.set_xticks(taus)
    ax.legend(loc="upper left")
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    plt.tight_layout()
    fig.savefig(fig_dir / f"fig_recovery_curves{fig_suffix}.pdf",
                dpi=150, bbox_inches="tight")
    fig.savefig(fig_dir / f"fig_recovery_curves{fig_suffix}.png",
                dpi=150, bbox_inches="tight")
    plt.close(fig)
    ...
    print(f"  Spearman(rel_pos, R_bad): rho={r_spear_bad:.4f}, "
          f"p={p_spear_bad:.2e}")

    fig, axes = plt.subplots(1, 2, figsize=(9, 4))

    ax = axes[0]
    tercile_means, tercile_ses, tercile_ns = [], [], []
    tercile_xlabels = []
    ...
    bars = ax.bar(range(3), tercile_means,
                  yerr=[1.96*s for s in tercile_ses],
                  capsize=5, color=colors_t, edgecolor="black",
                  linewidth=0.8, width=0.6)
    ax.set_xticks(range(3))
    ax.set_xticklabels(tercile_xlabels, fontsize=9)
    ax.set_ylabel("Repair Gain $\\Delta_{fix}$")
    ax.set_title("HumanEval: Repair Gain by Relative Position")
    for i, (bar, m, n) in enumerate(
            zip(bars, tercile_means, tercile_ns)):
        ax.text(bar.get_x() + bar.get_width()/2,
                bar.get_height() + 0.01,
    ...
    for i in range(n_bins):
        mask = ((rel_positions >= bin_edges[i])
                & (rel_positions < bin_edges[i+1]))
        if mask.sum() >= 5:
            bin_centers.append((bin_edges[i] + bin_edges[i+1]) / 2)
            bin_means.append(np.mean(deltas_all[mask]))
            bin_ses_plot.append(
                1.96 * np.std(deltas_all[mask]) / np.sqrt(mask.sum()))
    ax.errorbar(bin_centers, bin_means, yerr=bin_ses_plot,
    ...
    plt.tight_layout()
    fig.savefig(fig_dir / f"fig_relative_position{fig_suffix}.pdf",
                dpi=150, bbox_inches="tight")
    fig.savefig(fig_dir / f"fig_relative_position{fig_suffix}.png",
                dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: "
          f"{fig_dir / f'fig_relative_position{fig_suffix}.pdf'}")

    # Median-split Welch test
    ...
    t_rel, p_rel = sp_stats.ttest_ind(
        early_rel_delta, late_rel_delta, equal_var=False)
    ...
    if per_q:
        out_path = out_path.with_name(
            out_path.stem + "_perq" + out_path.suffix)
