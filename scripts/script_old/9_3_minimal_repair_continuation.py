#!/usr/bin/env python3
"""
Minimal repair continuation on MATH-500.
...
continue N times. Compare R_fix(t) against R_bad(t) from 9_2.

Multi-GPU data-parallel with checkpoint/resume support.

Usage:
    python scripts/9_3_minimal_repair_continuation.py \
        --early-file results/math500_3b_multi_sample/first_error/bucket_early.json \
        --late-file results/math500_3b_multi_sample/first_error/bucket_late.json \
        --out-dir results/math500_3b_multi_sample/minimal_repair \
        --n-continuations 32 \
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Minimal repair continuation (MATH-500)")
    ap.add_argument("--early-file", default=str(
        PROJECT_ROOT / "results/math500_3b_multi_sample/first_error/bucket_early.json"))
    ap.add_argument("--late-file", default=str(
        PROJECT_ROOT / "results/math500_3b_multi_sample/first_error/bucket_late.json"))
    ap.add_argument("--out-dir", default=str(
        PROJECT_ROOT / "results/math500_3b_multi_sample/minimal_repair"))
...
def _task_key(doc_id, sample_idx: int, continuation_idx: int) -> str:
