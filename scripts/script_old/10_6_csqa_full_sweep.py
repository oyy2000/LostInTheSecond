...
def vote(answers):
    letters = [a.strip().upper() for a in answers if a and a.strip()]
    return Counter(letters).most_common(1)[0][0] if letters else ""
...
def parse_args():
    ...
    ap.add_argument("--out-dir", default=str(ROOT / "results/csqa_full_sweep"))
        ...
        for task, output in zip(tasks, res):
            ...
            if tt == "draft":
                ...
                rec = {
                    ...
                    "draft_answer": pred,
                    "draft_correct": float(is_choice_correct(pred, gold)),
                    "draft_tokens": nt,
                }
            elif tt == "suffix":
                full = task["prefix_text"] + ("\n\n" + g if g else "")
                pred = extract_choice_letter(full)
                gold = task["gold_answer"]
                rec = {
                    "task_type": "suffix",
                    "doc_id": task["doc_id"],
                    ...
                    "gold_answer": gold,
                    "pred_answer": pred,
                    "exact_match": float(is_choice_correct(pred, gold)),
                    "suffix_tokens": nt,
...
def main():
    ...
    print("=" * 60)
    print(f"CommonsenseQA Full Sweep ({nq} questions)")
    ...
    greedy_tpq = (
        sum(drafts_idx[q["doc_id"]].get(0, {}).get("draft_tokens", 0) for q in questions) / nq
    )
    print(f"  Greedy: {greedy_acc:.4f} ({greedy_acc:.1%}), avg {greedy_tpq:.0f} tok/q")

    # ── Phase 2: Suffixes ─────────────────────────────────────────────────
    # K = number of suffix resamples PER draft
    n_sfx_per_draft = K_max
    ...
    for K in Ks:
        ...
        for q in questions:
            ...
            if is_choice_correct(vote(answers), q["gold_answer"]):
                nc += 1
        acc = nc / nq
        tpq = tt / nq
    ...
    # ── Print ──────────────────────────────────────────────────────────────
    print(f"\n{'=' * 90}")
    print(f"CommonsenseQA Full Sweep Results (n={nq}, greedy={greedy_acc:.1%}, {greedy_tpq:.0f} tok/q)")
    print(f"{'=' * 90}")
    print(f"{'Method':<16} {'nD':>3} {'K':>3} {'alpha':>6} {'Votes':>6} "
    ...
    for r in sorted(
        ...
        print(f"{'Full SC':<16} {'-':>3} {r['K']:>3} {'-':>6} {r['K']:>6} "
    ...
    for r in sorted(
        ...
        print(f"{'LR':<16} {r['n_drafts']:>3} {r['K']:>3} {r['alpha']:>6} {r['total_votes']:>6} "
              f"{r['accuracy']:>7.4f} {r['tpq']:>7.0f} "
              f"{r['gain']:>+7.4f} {r['multiplier']:>6.1f} {r['LTE']:>8.5f}")
    ...
    summary = {
        "model": args.model_id, "dataset": "CommonsenseQA",
        "n_questions": nq,
