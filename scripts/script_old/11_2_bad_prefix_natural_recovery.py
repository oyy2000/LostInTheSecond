#!/usr/bin/env python3
"""
Bad-prefix natural recovery on CodeContests.

For each wrong solution with a known first-error block (tau), keep the prefix
up to and including block tau, then let the base model continue N times.

Multi-GPU data-parallel via subprocess sharding.

Usage:
    python scripts/11_2_bad_prefix_natural_recovery.py \
        ...
        --out-dir results/codecontests_3b_multi_sample/bad_prefix_recovery \
        --n-continuations 32 \
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Bad-prefix natural recovery (CodeContests)")
    ...
    ap.add_argument("--out-dir", default=str(
        PROJECT_ROOT / "results/codecontests_3b_multi_sample/bad_prefix_recovery"))
