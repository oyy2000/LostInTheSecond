#!/usr/bin/env python3
"""
Combined analysis: Bad-prefix recovery vs Minimal repair on CommonsenseQA.
...
Requires outputs from 10_2 and 10_3.

Usage:
    python scripts/10_4_csqa_compare.py
    python scripts/10_4_csqa_compare.py --per-question
"""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Compare bad-prefix vs repair (CSQA)")
    ap.add_argument("--bad-file", default=str(
        PROJECT_ROOT / "results/csqa_3b_multi_sample/bad_prefix_recovery/continuations.jsonl"))
    ap.add_argument("--fix-file", default=str(
        PROJECT_ROOT / "results/csqa_3b_multi_sample/minimal_repair/continuations.jsonl"))
    ap.add_argument("--fig-dir", default=str(
        PROJECT_ROOT / "figures/csqa_bad_prefix_recovery"))
    ap.add_argument("--out-summary", default=str(
        PROJECT_ROOT / "results/csqa_3b_multi_sample/repair_gain_summary.json"))
...
def load_per_sample_rates(path: Path) -> dict:
    ...
    for key, recs in by_sample.items():
        ...
        n_steps = recs[0].get("n_blocks", recs[0].get("n_steps", 0))
        sample_rates[key] = {
...
def load_per_question_rates(path: Path) -> dict:
    ...
    for doc_id, recs in by_doc.items():
        ...
        n_steps_list = [r.get("n_blocks", r.get("n_steps", 0)) for r in recs]
...
def main():
    ...
    bad_rates = load_per_question_rates(bad_path) if per_q else load_per_sample_rates(bad_path)
    fix_rates = load_per_question_rates(fix_path) if per_q else load_per_sample_rates(fix_path)
    ...
    ax.bar(x - width/2, r_bad_means, width, yerr=[1.96*s for s in r_bad_ses],
           capsize=4, label="$R_{bad}$ (no repair)", color="#e74c3c", alpha=0.8)
    ax.bar(x + width/2, r_fix_means, width, yerr=[1.96*s for s in r_fix_ses],
           capsize=4, label="$R_{fix}$ (repaired)", color="#3498db", alpha=0.8)
    ax.set_xticks(x)
    ax.set_xticklabels(["early", "late"])
    ax.set_ylabel("Recovery Rate")
    ax.set_title(f"CommonsenseQA: Bad Prefix vs. Minimal Repair ({granularity})")
    ...
    ax.set_title("CommonsenseQA: $R_{fix} - R_{bad}$")
    ...
    ax.set_title(f"CommonsenseQA: Welch t={t_between:.2f}, p={p_between:.2e}")
    ax.axhline(0, color="gray", linestyle="--", linewidth=0.8)
    ...
    ax.set_ylabel("Recovery Rate")
    ax.set_title("CommonsenseQA: Recovery with and without Repair")
    ax.set_xticks(taus)
    ax.legend(loc="upper left")
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ...
    ax.set_ylabel("Repair Gain $\\Delta_{fix}$")
    ax.set_title("CommonsenseQA: Repair Gain by Relative Position")
