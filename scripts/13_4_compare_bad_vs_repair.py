#!/usr/bin/env python3
"""
Combined analysis: Bad-prefix recovery vs Minimal repair on MBPP.
...
Usage:
    python scripts/13_4_compare_bad_vs_repair.py
    python scripts/13_4_compare_bad_vs_repair.py --per-question
"""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Compare bad-prefix vs repair (MBPP)")
    ap.add_argument("--bad-file", default=str(
        PROJECT_ROOT / "results/mbpp_3b_multi_sample/bad_prefix_recovery/continuations.jsonl"))
    ap.add_argument("--fix-file", default=str(
        PROJECT_ROOT / "results/mbpp_3b_multi_sample/minimal_repair/continuations.jsonl"))
    ap.add_argument("--fig-dir", default=str(
        PROJECT_ROOT / "figures/mbpp_bad_prefix_recovery"))
    ap.add_argument("--out-summary", default=str(
        PROJECT_ROOT / "results/mbpp_3b_multi_sample/repair_gain_summary.json"))
...
def main():
    ...
    ax.set_title(f"MBPP: Bad Prefix vs. Minimal Repair ({granularity})")
    ...
    ax.set_title("MBPP: $R_{fix} - R_{bad}$")
    ...
    ax.set_title(f"MBPP: Welch t={t_between:.2f}, p={p_between:.2e}")
    ax.axhline(0, color="gray", linestyle="--", linewidth=0.8)
    ...
    ax.set_ylabel("Recovery Rate (pass@tests)")
    ax.set_title("MBPP: Recovery with and without Repair")
    ax.set_xticks(taus)
    ax.legend(loc="upper left")
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ...
    ax.set_ylabel("Repair Gain $\\Delta_{fix}$")
    ax.set_title("MBPP: Repair Gain by Relative Position")
