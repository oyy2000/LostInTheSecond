...
EXPERIMENT_MODE = "GPT_STEP2_FIX" 

# ==========================================
# 2. 公共配置 (两个实验共用)
# ==========================================
GPUS = [0, 1, 2, 3, 4, 5, 6, 7]  # 使用的 GPU 列表
TASKS = "hendrycks_math_500" # "gsm8k_cot_zeroshot_unified" 
...
elif EXPERIMENT_MODE == "GPT_STEP2_FIX":
    # New mode for GPT Step2 Fix datasets
    DATASET_VARIANT = "fix_step2"  # or "fix_step2" wait_recompute
    NUM_EXAMPLES = 16
    EXTRACTION_VARIANT = "step2_marker"
    VECTOR_DIR = os.path.join(
        "./artifacts",
        f"vectors_{NUM_EXAMPLES}_ds2_{DATASET_VARIANT}_incorrect_only_id300+_{EXTRACTION_VARIANT}",
        applied_sanitized + "_applied"
    )
    # 输出后缀 & 运行参数
    outdir_suffix = f"_step2fix_{DATASET_VARIANT}"
