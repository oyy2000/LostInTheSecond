#!/usr/bin/env python3
"""
Minimal-repair continuation on HumanEval.

Replace block tau with GPT's correction, let the model continue N times.

Usage:
    python scripts/12_3_minimal_repair_continuation.py \
...
from src.code_answer_equiv import check_humaneval, extract_python_code
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--early-file", default=str(
        PROJECT_ROOT / "results/humaneval_3b_multi_sample/first_error/bucket_early.json"))
    ap.add_argument("--late-file", default=str(
        PROJECT_ROOT / "results/humaneval_3b_multi_sample/first_error/bucket_late.json"))
    ap.add_argument("--out-dir", default=str(
        PROJECT_ROOT / "results/humaneval_3b_multi_sample/minimal_repair"))
...
def main() -> None:
    ...
    for s in samples:
        ...
        prompt_text = build_chat_prompt(s["prompt"], prefix_code)
