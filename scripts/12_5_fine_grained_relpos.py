#!/usr/bin/env python3
"""
Fine-grained relative-position analysis on HumanEval.
...
Usage:
    python scripts/12_5_fine_grained_relpos.py
"""
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--out-summary", default=str(
        PROJECT_ROOT / "results/humaneval_3b_multi_sample/fine_relpos_summary.json"))
...
def main():
    ...
    for key in common:
        ...
        if n_blocks <= 0 or tau / n_blocks >= 1.0:
            continue
    ...
    ax.set_title(
        f"HumanEval: Repair Gain by Position (KW p={kw_p:.2e})")
    ...
    ax.set_title(
        f"HumanEval: Spearman $\\rho_s$={rho:.3f}, p={p_rho:.1e}")
    ax.axhline(0, color="gray", linestyle="--", linewidth=0.8)
    ...
    ax.set_title("HumanEval: Pairwise Welch t-test p-values")
    plt.tight_layout()
