#!/usr/bin/env python3
"""
Experiment: Rollback-one-step recovery.

For each wrong trajectory with a known first-error step (tau), roll back to
the end of step tau-1 (the last CORRECT step) and resample from there.
This is a clean "oracle rollback" baseline: the model never sees the bad step.

Contrast with 6_1 (bad prefix includes the error step) and 6_3 (error step
is replaced by a GPT correction).

Measure R_rollback(t) = P(correct | prefix up to step tau-1).
...
Usage:
    python scripts/6_5_rollback_one_step_recovery.py \
        ...
        --out-dir results/gsm8k_3b_multi_sample/rollback_one_step \
        --n-continuations 32 \
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--out-dir", default=str(
        PROJECT_ROOT / "results/gsm8k_3b_multi_sample/rollback_one_step"))
...
def build_prefix_text(steps: List[str], tau: int) -> str:
    """Join steps[0..tau-2]: the correct prefix BEFORE the first error step.

    tau is 1-indexed, so steps[tau-1] is the error step.
    We keep steps[:tau-1], rolling back to just before the error.
    """
    prefix_steps = steps[:tau - 1]
    return "\n\n".join(prefix_steps)
...
def main() -> None:
    ...
    print("\n=== Recovery rates (rollback one step) ===")
    for bucket in ["early", "late"]:
        vals = by_bucket[bucket]
        if vals:
            rate = sum(vals) / len(vals)
            print(f"  R_rollback({bucket}): {rate:.4f} ({sum(int(v>=1) for v in vals)}/{len(vals)})")
