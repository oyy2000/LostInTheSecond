#!/usr/bin/env python3
"""
Budget-controlled rollback with a uniformly random rollback signal.

This rebuttal experiment changes only the rollback-location signal from
6_8_budget_controlled_multisignal.py. For each draft, random_uniform
samples the number of retained steps uniformly from [0, n_steps - 1].
Suffix generation, the nd * ns = budget constraint, and majority-vote
evaluation remain identical to the source experiment.

Usage:
    python rebuttal/scripts/6_8_budget_controlled_random.py \\
        --dataset aime2025 --budget 32 --gpus 0,1,2 \\
        --signals random_uniform --random-seed 42 \\
        --draft-checkpoint \\
          results/llama_3.2_3b_instruct_budget_multisignal/aime2025/checkpoint.jsonl
"""
...
REBUTTAL_ROOT = PROJECT_ROOT / "rebuttal"
...
def parse_args():
    ...
    ap.add_argument(
        ...
        help="Seed for deterministic per-draft random rollback locations")
...
def compute_rollback_step_random(doc_id, draft_idx, n, seed):
    """Uniformly sample a valid retained-prefix length for one draft.

    A content-derived seed makes the result stable across question shards,
    process counts, and checkpoint ordering.
    """
    if n <= 1:
        return 0
    key = f"{seed}|{doc_id}|{draft_idx}".encode("utf-8")
    value = int.from_bytes(hashlib.sha256(key).digest()[:8], "big")
    return random.Random(value).randrange(n)
...
def main():
    ...
    print(f"Random rollback seed: {args.random_seed}")
    print(f"PRM model: {args.prm_model}"
          f"{' (Skywork 1.5B)' if use_skywork else ''}")
    (out_dir / "run_config.json").write_text(
        json.dumps({
            "source_script": "scripts/6_8_budget_controlled_multisignal.py",
            "dataset": DATASET,
            "model": MODEL_ID,
            "budget": B,
        ...
        if ext_path.exists():
            ...
            with ckpt.open("a") as f:
                for line in ext_path.open(encoding="utf-8"):
                    ...
                    if r.get("task_type") != "draft":
                        continue
                    if r.get("doc_id") not in q_map:
                        continue
                    if int(r.get("draft_idx", -1)) >= nd_max:
                        continue
    ...
    for q in questions:
        for di in range(nd_max):
            ...
            if "random_uniform" in signals:
                rb_steps["random_uniform"] = (
                    compute_rollback_step_random(
                        q["doc_id"], di, n, args.random_seed))
                ...
                for strat, rb in rb_steps.items():
                    ...
                    for si in range(ns_for_draft[di]):
                        ...
                        sfx_tasks.append(dict(
                            ...
                            random_seed=(
                                args.random_seed
                                if strat == "random_uniform" else None),
                            rollback_step=b,
                            suffix_idx=si,
                            gold_answer=q["gold_answer"],
                            prompt=p,
